...
async def get_pool_coins(session: aiohttp.ClientSession, pool_id: str):
    ...
    print(f"Pool {pool_id}: coin_a={coin_type_a}, coin_b={coin_type_b}")
...
async def fetch_bluefin_swap_events_paginated(
    ...
    """
    Asynchronously fetch up to `pages` pages of bluefin SwapEvent objects.
    """
    event_filter = {
        "MoveEventType": "0x3b6d71bdeb8ce5b06febfd3cfc29ecd60d50da729477c8b8038ecdae34541b91::bluefin::BluefinSwapEvent" # Bluefin All Swaps
    }
...
async def _fill_single_event_from_cache(event: dict):
    ...
    a_to_b = pjson.get("a2b", pjson.get("a_to_b", None))
    ...
    # Populate event with enriched data
    ...
    # Debugging: Log the enrichment details
    print(f"Enriched event {event['id']['eventSeq']}: token_in={token_in} ({in_symbol}), token_out={token_out} ({out_symbol})")
...
async def enrich_events_with_all_data(session, events):
    ...
    # Collect unique coin types from pools
...
def write_events_to_csv_with_enrichment(events, output_csv="bluefin_swap_events.csv"):
    """
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        events = await fetch_bluefin_swap_events_paginated(
            session,
            pages=DEFAULT_PAGES,
            limit_per_page=DEFAULT_LIMIT_PER_PAGE,
            descending=DEFAULT_DESCENDING
    ...
    start_csv = time.time()
    output_csv = "bluefin_swap_events.csv"
