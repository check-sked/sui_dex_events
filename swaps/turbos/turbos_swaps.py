...
# Updated event type for the new package
EVENT_TYPE = "0x91bfbc386a41afcfd9b2533058d7e915a1d3829089cc268ff4333d54d6339ca1::pool::SwapEvent"
...
async def get_pool_coins(session: aiohttp.ClientSession, pool_id: str):
    ...
    # Updated parsing logic for the pool structure
    type_str = obj_data.get("type", "")
    if "Pool<" in type_str:
        inside = type_str.split("Pool<", 1)[1].rstrip(">")
        parts = [p.strip() for p in inside.split(",")]
        if len(parts) >= 2:  # Changed to handle 3 parts (coin_a, coin_b, fee_type)
            coin_type_a, coin_type_b = parts[0], parts[1]

    content = obj_data.get("content", {})
    fields = content.get("fields", {})
    if not coin_type_a:
        coin_type_a = fields.get("coin_type_a", fields.get("coin_a_type"))
    if not coin_type_b:
        coin_type_b = fields.get("coin_type_b", fields.get("coin_b_type"))
...
async def fetch_swap_events_paginated(
    ...
    """
    Asynchronously fetch up to `pages` pages of SwapEvent objects.
    """
    event_filter = {
...
###############################################################################
# 6) BATCHED ENRICHMENT
###############################################################################

async def enrich_events_with_pool_and_coin_data(session, events):
    """
    Fetch pool and coin metadata in parallel batches
    """
...
async def _fill_single_event_from_cache(event: dict):
    """
    Fill event details from cached data (no new network calls).
    """
    pjson = event.get("parsedJson", {})
    # Get pool ID from either the pool field or find it in objectChanges
    pool_id = pjson.get("pool", "")
    if not pool_id:
        # Try to find pool ID from object changes
        obj_changes = event.get("objectChanges", [])
        for change in obj_changes:
            if change.get("objectType", "").startswith("0x91bfbc386a41afcfd9b2533058d7e915a1d3829089cc268ff4333d54d6339ca1::pool::Pool"):
                pool_id = change.get("objectId", "")

    coin_a, coin_b = _pool_cache.get(pool_id, ("Unknown", "Unknown"))
    # Handle both atob and a_to_b formats
    a_to_b = pjson.get("atob", pjson.get("a_to_b", None))

    # Handle both amount_in/out and amount_a/b formats
    raw_amt_in = pjson.get("amount_in", "")
    raw_amt_out = pjson.get("amount_out", "")
    raw_amt_a = pjson.get("amount_a", "")
    raw_amt_b = pjson.get("amount_b", "")
    ...
    if a_to_b is True:
        token_in = coin_a
        token_in_raw = raw_amt_a if "amount_a" in pjson else raw_amt_in
        token_out = coin_b
        token_out_raw = raw_amt_b if "amount_b" in pjson else raw_amt_out
    elif a_to_b is False:
        token_in = coin_b
        token_in_raw = raw_amt_b if "amount_b" in pjson else raw_amt_in
        token_out = coin_a
        token_out_raw = raw_amt_a if "amount_a" in pjson else raw_amt_out
...
def write_events_to_csv_with_enrichment(events, output_csv="swap_events.csv"):
    """
