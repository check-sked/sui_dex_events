...
DEX_CONFIGS = {
    "bluefin": {
        "name": "Bluefin",
        "package_id": "0x3492c874c1e3b3e2984e8c41b589e642d4d0a5d6459e5a9cfc2d52fd7c89c267",
        "event_query": {
            "MoveEventType": "0x3492c874c1e3b3e2984e8c41b589e642d4d0a5d6459e5a9cfc2d52fd7c89c267::events::PoolCreated"
        }
    }
}
    ...
    while True:
        try:
            payload = {
                "jsonrpc": "2.0",
                "id": 1,
                "method": "suix_queryEvents",  # Ensure this is correct
                "params": [
                    event_filter,
                    cursor,
                    100,
            ...
            if not data.get("data"):
                print("No events found. Full RPC response:")
                print(json.dumps(result, indent=2))
                break

            print(f"Fetched {len(data.get('data', []))} events from RPC.")

            for event in data.get("data", []):
                try:
                    print("Raw event data:", json.dumps(event, indent=2))  # Debugging

                    parsed_json = event.get("parsedJson", {})
                    timestamp_ms = int(event.get("timestampMs", 0))  # Ensure correct key

                    # **Updated Pool ID Extraction**
                    pool_id = parsed_json.get("pool_id") or parsed_json.get("poolId") or parsed_json.get("id")
                    if not pool_id:
                        print(f"Available keys in parsedJson: {list(parsed_json.keys())}")
                        print("Pool ID not found in event. Skipping event.")
                        continue
                    ...
                    print(f"Successfully extracted pool ID: {pool_id} (created at: {datetime.fromtimestamp(timestamp_ms/1000).isoformat()})")

                    # Also print coin information for debugging
                    # Adjusting to match the structure of parsedJson
                    coin_a = parsed_json.get("coin_a")
                    coin_b = parsed_json.get("coin_b")
                    print(f"Pool coins: {coin_a}, {coin_b}")
                except Exception as e:
...
async def get_pool_details(session: aiohttp.ClientSession, pool_id: str) -> Tuple[dict, Tuple[str, str]]:
    ...
    try:
        ...
        if not data:
            ...
            return {"fee_percent": None, "weight_a": 0.5, "weight_b": 0.5}, ("Unknown", "Unknown")

        content = data.get("content", {})
        fields = content.get("fields", {})

        pool_details = {
            'fee': fields.get('fee_rate', None),  # Updated key to match event data
...
async def get_coin_metadata(session: aiohttp.ClientSession, coin_type: str) -> Tuple[int, str]:
    ...
    payload = {
        "jsonrpc": "2.0",
        "id": 1,
        "method": "suix_getCoinMetadata",  # Ensure this is correct
        "params": [coin_type]
    }
